...
from eval.eval_io import (
    _differing_keys, _hp_row, best_checkpoint_path,
    find_run_dirs, load_run_config, save_results_csv,
)
from eval.solver import (
    UniformActor, augment_coords, autocast_for, build_actor_from_config,
    run_sa, set_seed, strip_enrichment, warmup_cuda,
)
...
def add_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--greedy", dest="GREEDY",
                        action="store_true", default=False)
    parser.add_argument("--no-metro", dest="METROPOLIS",
                        action="store_false", default=True)
    parser.add_argument("--augment", type=int, default=1,
                        choices=range(1, 9), metavar="K",
...
def run(args: argparse.Namespace) -> None:
    ...
    fake_cfg = {
        "eval":    {"dim": args.dim, "n_problems": args.batch_size,
    ...
    if n_problems != args.batch_size:
        logger.info(f"Effective n_problems = {n_problems} (requested {args.batch_size}).")
    ...
    for run_dir, cfg in tqdm(list(zip(run_dirs, run_cfgs)),
                              desc="Models", unit="run"):
            ...
            if args.BASELINE:
                ...
                with torch.inference_mode():
                    bl_res = run_sa(
                        ...
                        greedy=False, metropolis=args.METROPOLIS,
                    )
                baseline_time = time.perf_counter() - t0
                baseline_cost = bl_res["best_cost"].mean().item()

            # 3b. Learned arm with D4 ensemble.
            best_per_problem = torch.full(
                (n_problems,), float("inf"), device=device
            )
            t0 = time.perf_counter()
            for k in range(args.augment):
                ...
                with torch.inference_mode():
                    res_k = run_sa(
                        actor, state_k, cfg,
                        outer_steps=args.OUTER_STEPS,
                        greedy=args.GREEDY, metropolis=args.METROPOLIS,
    ...
    fixed = [
        "run", "ckpt", "dtype", "dim", "max_load", "n_problems",
        "outer_steps", "augment", "initial_cost", "final_cost",
        "baseline_cost", "exec_time", "baseline_time",
    ]
    swept_cols = sorted(diff_keys)
    df = pd.DataFrame(rows, columns=fixed + swept_cols)
    out = save_results_csv(
        df, args.project, args.group,
